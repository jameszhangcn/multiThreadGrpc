...
using grpc::Status;
using srvCp::SecurityConfigureRsp;
...
class CuCpClient {
 public:
  ...
  std::string SecurityConfigureCfm() {
    ...
    hdr->mutable_fromloc()->set_address("10.2.55.118");
    hdr->mutable_fromloc()->set_port(9020);

    hdr->mutable_torid()->set_sn("CUCP");
    hdr->mutable_toloc()->set_address("10.2.55.118");
...
int main(int argc, char** argv) {
  ...
  CuCpClient ebm(grpc::CreateChannel(
      "10.2.55.118:9021", grpc::InsecureChannelCredentials()));
