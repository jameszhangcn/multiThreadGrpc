...
public: \
    _OBJ_NAME(std::shared_ptr<grpc::Channel> aChannel, std::shared_ptr<grpc::CompletionQueue> aCq) \
        ...
        , mCq(aCq) {} \
 \
    ~_OBJ_NAME() {printf("Destroy OBJ %s %x \n", #_OBJ_NAME, this);}  \
...
public:
    ...
    std::shared_ptr<grpc::Channel> getChannel() {printf("getChannel %x", &mChannel[0]); return mChannel[0];};
    std::shared_ptr<grpc::Channel> getChannel(int aId) {printf("getChannel %x", &mChannel[aId]);return mChannel[aId];};
    std::shared_ptr<grpc::CompletionQueue> getCompletionQueue() {printf("getCmq %x ", &mCq);return mCq;};
