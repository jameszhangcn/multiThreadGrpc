...
namespace mav
{
    ...
    int GrpcAdpCntrl::createGrpcClient(string cuCpID, long cuCpInst, string cuUpID)
    {
        ...
        grpcClient->init(threadInstanceId);
        grpcClient->run();
        printf("createGrpcClient cuCpID %s cuCpInst %x cuUpID %s ", cuCpID.c_str(), cuCpInst, cuUpID.c_str());
        int index = get_free_client_idx(grpcClient);
        printf("createGrpcClient free idx %d", index);
        mEbmClientMap.insert(pair<EbmGrpcClient,int>(record, index));;
    }
